...
(function () {
  ...
  function UserService($http,$rootScope,toastr) {
    var service = {
      ...
      updateUserForFanHeader: updateUserForFanHeader,
    ...
    function updateUserForFanHeader(user){
        //console.warn(user);
        return $http.put(userURL + '/' + user.id, user)
          .then(updateUserForFanHeaderComplete)
          .catch(updateUserForFanHeaderFailed);
        function updateUserForFanHeaderComplete(response){
          return response.data;
        }
        function updateUserForFanHeaderFailed(error){
          console.error('XHR Failed for updateUserFailed.' + error.data);
        }
      }
